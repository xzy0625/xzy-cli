"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const index_1 = require("./utils/index");
const commander_1 = require("commander");
const logger_1 = require("./helpers/logger");
const chalk = require("chalk");
const figlet = require("figlet");
const printLastVersion = async (version, name) => {
    const latestVersion = await (0, index_1.getLatestVersion)(name);
    if (latestVersion === version || true) {
        logger_1.default.info(`当前包有最新版本，可更新版本，${chalk.green(version)} -> ${chalk.green(latestVersion)} \n执行npm install -g ${name}`);
    }
};
const start = async () => {
    // 获取所有命令
    const commandsPath = await (0, index_1.getPathList)("./commands/*.*s");
    // 注册命令
    commandsPath.forEach((commandPath) => {
        const commandObj = require(`./${commandPath}`);
        const { command, description, optionList, action } = commandObj.default;
        const curp = commander_1.program
            .command(command)
            .description(description)
            .action(action);
        optionList &&
            optionList.map((option) => {
                curp.option(...option);
            });
    });
    const packageInfo = (0, index_1.getPkgInfo)();
    const { version, name } = packageInfo;
    // 配置版号，执行zy --version显示版本
    commander_1.program.version(version);
    // 监听 --help 指令,加上额外的提示
    commander_1.program.on("--help", async function () {
        // 美化logo
        console.log("\r\n" +
            figlet.textSync("xzy-cli", {
                font: "3D-ASCII",
                horizontalLayout: "default",
                verticalLayout: "default",
                width: 80,
                whitespaceBreak: true,
            }));
        // 前后两个空行调整格式，更舒适
        console.log();
        console.log(`Run ${chalk.cyan("zc-cli <command> --help")} for detailed usage of given command.`);
        console.log();
    });
    commander_1.program.on("command:*", async ([cmd]) => {
        commander_1.program.outputHelp();
        logger_1.default.error(`未知命令 command ${chalk.yellow(cmd)}.`);
        await printLastVersion(version, name);
        process.exitCode = 1;
    });
    // 调用参数解析去匹配命令
    commander_1.program.parseAsync(process.argv);
};
start();
